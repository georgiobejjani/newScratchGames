...
scratchImage.src = "./assetsBingoSafari/ScratchAreaAlphabet.png";
...
function getInfos() {
  ...
  let pagesize = 1000;
  let pagenb = 0;
  let url = `${apiEndpoint}scratch-api/tokens/${token}/tickets?Type=${type}&isFinished=1&PageNumber=${pagenb}&PageSize=${pagesize}`;
  fetch(url)
    .then((response) => {
      if (!response.ok) {
        throw new Error("Network response was not ok");
