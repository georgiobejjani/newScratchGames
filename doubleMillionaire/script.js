...
function onBuyScratch(number) {
  ...
  fetch(
    "https://api.lonaci-moja.ci/scratch-api/tickets?isWallet=true&language=en",
    {
      method: "POST",
      headers: {
...
function getUnplayed() {
  ...
  let url = `https://api.lonaci-moja.ci/scratch-api/tokens/${token}/tickets?Type=${type}&PageNumber=${pagenb}&PageSize=${pagesize}`;
  fetch(url)
    .then((response) => {
      if (!response.ok) {
        throw new Error("Network response was not ok");
...
function playTicket(ticketId) {
  ...
  fetch("https://api.lonaci-moja.ci/scratch-api/tickets?language=en", {
    method: "PATCH",
    headers: {
      "Content-Type": "application/json",
    },
...
//   animationSprite.style.display = 'block';
//   animate();
// }
...
//   var currentFrame = parseInt(animationSprite.style.backgroundPositionX) || 0;

//   currentFrame = (currentFrame - frameWidth) % (frameWidth * totalFrames);

//   animationSprite.style.backgroundPositionX = currentFrame + 'px';

//   if (currentFrame > -frameWidth * (totalFrames - 1)) {
//     setTimeout(function () {
//       requestAnimationFrame(animate);
//     }, animationSpeed);
//   } else {
//     // Animation complete, hide the sprite
//     animationSprite.style.display = 'none';
//   }
// }
